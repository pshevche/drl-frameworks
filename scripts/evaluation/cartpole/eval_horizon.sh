...
for fullfile in experiments/cartpole/horizon/cpu/*.json; do 
    ...
    python src/drl_fw/horizon/run_evaluation.py -p experiments/cartpole/horizon/cpu/${experiment}.json -f results/cartpole/${experiment}/checkpoints.json -v results/cartpole/
...
for fullfile in experiments/cartpole/horizon/gpu/*.json; do 
    ...
    echo "--- STARTING EXPERIMENT ${experiment} --- "
    mkdir -p results/cartpole/${experiment}
    python src/drl_fw/horizon/run_evaluation.py -g 0 -p experiments/cartpole/horizon/gpu/${experiment}.json -f results/cartpole/${experiment}/checkpoints.json -v results/cartpole/
    echo "--- EXPERIMENT ${experiment} COMPLETED --- "
    echo
done
conda deactivate
